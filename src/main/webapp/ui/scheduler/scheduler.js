    ...
    if (resource) {
      return this.clientEvents((function(_this) {
        return function(event) {
          return $.inArray(resource.resourceId, _this.getEventResourceIds(event)) !== -1;
        };
      })(this));
    } else {
      return [];
...
ResourceManager = (function(superClass) {
  ...
  ResourceManager.prototype.addResourceToIndex = function(resource) {
    var child, j, len, ref;
    if (this.resourcesById[resource.resourceId]) {
      return false;
    } else {
      this.resourcesById[resource.resourceId] = resource;
  ...
  ResourceManager.prototype.removeResource = function(idOrResource) {
    var id;
    id = typeof idOrResource === 'object' ? idOrresource.resourceId : idOrResource;
    return syncThen(this.fetching, (function(_this) {
      return function() {
        var resource;
  ...
  ResourceManager.prototype.buildResource = function(resourceInput) {
    ...
    resource.resourceId = String((ref = resourceInput.id) != null ? ref : '_fc' + ResourceManager.resourceGuid++);
    ...
    if (this.resourceTextFunc) {
      ...
      if (typeof func !== 'function') {
        func = function(resource) {
          return resource.resourceName || resource.resourceId;
        };
      }
...
ResourceDayTableMixin = {
  ...
  renderHeadResourceCellHtml: function(resource, date, colspan) {
    return '<th class="fc-resource-cell"' + ' data-resource-id="' + resource.resourceId + '"' + (date ? ' data-date="' + date.format('YYYY-MM-DD') + '"' : '') + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>' + htmlEscape(this.view.getResourceText(resource)) + '</th>';
  },
  renderHeadResourceDateCellHtml: function(date, resource, colspan) {
    return this.renderHeadDateCellHtml(date, colspan, 'data-resource-id="' + resource.resourceId + '"');
  },
  ...
  renderResourceBgCellHtml: function(date, resource) {
    return this.renderBgCellHtml(date, 'data-resource-id="' + resource.resourceId + '"');
  },
...
ResourceTimelineView = (function(superClass) {
  ...
  ResourceTimelineView.prototype.removeResource = function(resource) {
    var row;
    row = this.getResourceRow(resource.resourceId);
    if (row) {
      this.batchRows();
      row.remove();
      this.unbatchRows();
  ...
  ResourceTimelineView.prototype.rowAdded = function(row) {
    ...
    if (row instanceof ResourceRow) {
      this.resourceRowHash[row.resource.resourceId] = row;
  ...
  ResourceTimelineView.prototype.rowRemoved = function(row) {
    var isNesting, wasNesting;
    if (row instanceof ResourceRow) {
      delete this.resourceRowHash[row.resource.resourceId];
...
ResourceTimelineGrid = (function(superClass) {
  ...
  ResourceTimelineGrid.prototype.queryHit = function(leftOffset, topOffset) {
    ...
    if (simpleHit) {
      ...
      if (rowIndex != null) {
        return {
          resourceId: this.shownEventRows[rowIndex].resource.resourceId,
          snap: simpleHit.snap,
          component: this,
          left: simpleHit.left,
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (rowObj.resource) {
        ...
        if (elBottom > scrollerTop) {
          state.resourceId = rowObj.resource.resourceId;
  ...
  ResourceTimelineGrid.prototype.scrollToResource = function(resource) {
    ...
    row = this.view.getResourceRow(resource.resourceId);
    if (row) {
      el = row.getTr('event');
      if (el) {
        innerTop = this.bodyScroller.canvas.el.offset().top;
...
ResourceRow = (function(superClass) {
  ...
  ResourceRow.prototype.renderEventContent = function(tr) {
    ...
    return tr.attr('data-resource-id', this.resource.resourceId);
  };
  ...
  ResourceRow.prototype.renderSpreadsheetContent = function(tr) {
    ...
    return tr.attr('data-resource-id', resource.resourceId);
  };
