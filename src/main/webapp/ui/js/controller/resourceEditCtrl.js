...
resourceEdit.controller('resourceEditCtrl', function($scope, $http, $window, $filter, utilityFunctions, userDetails, Notification) {
    ...
    $scope.removeAdmin = function() {
        ...
        if ($("#resourceAdmin option:selected").index() >= 0) {
            $.each($("#resourceAdmin option:selected").get().reverse(), function() {
            	var obj = $filter('filter')($scope.resource.resourceAdmins, {
                    email: $(this).val()
                }, true)[0];
            	var index = $scope.resource.resourceAdmins.indexOf(obj);
            ...
            Notification.warning({
                message: "Select an resource admin",
                delay: 2000
            });
        }
    ...
    $scope.addAdmin = function() {
        ...
        if ($("#userList option:selected").index() >= 0) {
            $.each($("#userList option:selected").get().reverse(), function() {
            	var obj = $filter('filter')($scope.allUsers, {
                    email: $(this).val()
                }, true)[0];
            	var index = $scope.allUsers.indexOf(obj);
