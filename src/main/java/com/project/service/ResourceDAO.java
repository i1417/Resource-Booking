...
public class ResourceDAO {
	...
	public boolean editResource(ResourcesModel resourceModel) {
		...
		try {
			...
			ResourcesModel objectToUpdate = (ResourcesModel) session.get(ResourcesModel.class, resourceModel.getResourceId());
	...
	public boolean addResourceAdmin(ResourcesModel resourceModel, UsersModel userModel) {
		Session session = sessionFactory.openSession();
		...
		try {
			List<ResourcesModel> resourceAdminList = objectToUpdate.getAdminOfResources();
			boolean result = true;
			for (ResourcesModel resourcesModel : resourceAdminList) {
				if(resourcesModel.getResourceId() == resourceModel.getResourceId()) {
					result = false;
				}
			}
			if(result) {
	...
	public boolean deleteResourceAdmin(ResourcesModel resourceModel) {
		...
		try {
			...
			List<UsersModel> updatedListOfAdmin = resourceModel.getResourceAdmins();
			...
			for (int i = 0; i < listOfAdmin.size(); i++) {
				...
				for (UsersModel newUsersModel : updatedListOfAdmin) {
					if(newUsersModel.getEmployeeId() == userModel.getEmployeeId()) {
						flag = false;
						break;
					}
				}
